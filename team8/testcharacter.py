...
class TestCharacter(CharacterEntity):
    ...
    def get_q_reward(self,wrld, best_move):
        if best_move ==(7,18):
            ...
            exit_reward = 1/(1+self.get_Gn(best_move,wrld.exitcell))
    ...
    def get_wall_distance(self,wrld, position, move_direction):
        print(position)
        if move_direction==1:
            for i in range(position[0], wrld.width(),1):
                x = (position[0]+i)
                y = position[1]
                if x<=(wrld.width()-1):
                    if wrld.wall_at(x,y):
        ...
        if move_direction==2:
            for i in range(position[1], wrld.height(),1):
                x = position[0]
                y = (position[1]+i)
        ...
        if move_direction==3:
            for i in range(0,position[0],1):
                x = i
                y = position[1]
                # print("x =",x, " y = ", y)
                # print(x<=(wrld.width()-1))
                if x<=(position[0]-1):
                    # print(wrld.wall_at(x,y))
                    if wrld.wall_at(x,y):
                        return position[0]-i
            else:
                return position[0]
